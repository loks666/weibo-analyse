...
def getContentCloud():
    ...
    string = ' '.join(newCut)
    img = Image.open('content.jpg')  # 打开遮罩图片
    ...
    # 输入词语图片到文件
    plt.savefig('contentCloud.jpg', dpi=500)
...
def getCommentContentCloud():
    ...
    for word in cut:
        if word not in stopwords: newCut.append(word)
    string = ' '.join(newCut)
    img = Image.open('comment.jpg')  # 打开遮罩图片
    img_arr = np.array(img)  # 将图片转化为列表
    wc = WordCloud(
        width=1000, height=600,
        background_color='white',
    ...
    plt.savefig('commentCloud.jpg', dpi=500)
